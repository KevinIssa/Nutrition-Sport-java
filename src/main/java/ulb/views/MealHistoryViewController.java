...
public class MealHistoryViewController implements ViewController {
	...
	private ConsumedFoodDTO createConsumedFoodDTO(HBox foodBox) {
		...
		double quantity = Double.parseDouble(((Label) foodBox.getChildren().get(2)).getText().split(" ")[0]);
		double calories = Double.parseDouble(((Label) foodBox.getChildren().get(3)).getText().split(" ")[0]);
	...
	private void handleException(Exception e) {
        switch (e) {
            ...
            case NumberFormatException numberFormatException -> logger.error("Error parsing number: " + e.getMessage());
            case DateTimeParseException dateTimeParseException -> logger.error("Error parsing date: " + e.getMessage());
            case ClassCastException classCastException -> logger.error("Error casting class: " + e.getMessage());
