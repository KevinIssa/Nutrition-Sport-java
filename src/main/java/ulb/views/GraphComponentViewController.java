...
public class GraphComponentViewController extends AnchorPane{
    private static final Logger logger = LoggerFactory.getLogger(GraphComponentViewController.class);
    @FXML
    ...
    private void setupGraph() { // not put in the Initialize method because need the listener to be set
    ...
    private void initLineChart(){
        ...
        yAxis.setLabel("Calories");
        for (DateCalorieDTO dateCalorie : data) {
            series.getData().add(new XYChart.Data<>(dateCalorie.date().toString(), dateCalorie.calorieIntake()));
            series2.getData().add(new XYChart.Data<>(dateCalorie.date().toString(), dateCalorie.calorieBurned()));
        }
